...
export class TodosOverviewBloc extends Bloc<TodosOverviewEvent, TodosOverviewState> {
  #todosRepository: TodosRepository;
  ...
  constructor({ todosRepository }: TodosOverviewBlocProps) {
    ...
    this.on(TodosOverviewClearCompletedRequested, this.onClearCompletedRequested);
  ...
  async onToggleAllRequested(
    ...
    const areAllCompleted = this.state.data.todos.every((todo) => todo.isCompleted);
    await this.#todosRepository.completeAll(!areAllCompleted);
  }

  onFilterChanged(event: TodosOverviewFilterChanged, emit: Emitter<TodosOverviewState>) {
    emit((state) =>
      state.ready((data) => {
